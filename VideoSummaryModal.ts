...
export class VideoSummaryModal extends Modal {
  ...
  onOpen() {
    ...
    const localFileSetting = new Setting(contentEl)
      ...
      .setDesc('Docker/n8n 场景建议填 uploads 目录内的文件名或相对路径；多个用逗号分隔。');

    this.localFileInput = localFileSetting.controlEl.createEl('input');
    this.localFileInput.type = 'text';
    this.localFileInput.placeholder = 'lecture-01.mp4, uploads/lecture-02.mp4';
  ...
  async processVideo() {
    ...
    try {
      ...
      if (this.localFileInput && this.localFileInput.value.trim()) {
        const files = this.localFileInput.value
          .split(',')
          .map((f) => f.trim())
          .filter((f) => f);
        if (files.length > 0) {
      ...
      if (
        ...
        !videoInput.localFile &&
        (!videoInput.localFiles || videoInput.localFiles.length === 0)
      ) {
      ...
      if (videoInput.localFiles && videoInput.localFiles.length > 1 && videoInput.merge) {
        ...
        this.progressNotice = new Notice(
          `正在合并处理 ${videoInput.localFiles.length} 个文件...`,
          0,
        );
        ...
        try {
          ...
          for (let i = 0; i < videoInput.localFiles.length; i++) {
            ...
            if (this.progressNotice) {
              this.progressNotice.setMessage(
                `正在提取第 ${i + 1}/${videoInput.localFiles.length} 个文件的内容...`,
              );
            }
          ...
          const result = await RetryUtils.withSmartRetry(
            ...
            2000,
            (attempt, error) => {
              if (this.progressNotice) this.progressNotice.setMessage(`重试中: ${error.message}`);
            },
          );
