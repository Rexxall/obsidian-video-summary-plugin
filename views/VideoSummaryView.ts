import { ItemView, WorkspaceLeaf, TFile, Notice, Modal, TFolder } from 'obsidian';
...
export class VideoSummaryView extends ItemView {
  ...
  private looksLikeAbsoluteLocalPath(value: string): boolean {
  ...
  private renderCombinedInputSection(container: HTMLElement) {
    ...
    const selectFileBtn = localFileContainer.createEl('button', {
      ...
      attr: {
        type: 'button',
        style:
          'margin: 0 0 0 6px; height: 28px; line-height: 1; padding: 0 12px; width: auto; min-width: 0; flex: none !important; font-size: 13px; white-space: nowrap;',
      },
    });
    ...
    addToQueueBtn.onclick = async () => {
      ...
      const singleUrl =
        isSingleLink && !isJsonMode && !multiPCheck.checked
      ...
      if (isSingleLink) {
        ...
        if (isMultiP) {
          ...
          if (useRange) {
            ...
            if (startP < 1 || endP < 1) {
              new Notice('分P范围必须大于0');
              return;
            }
            if (startP > endP) {
              new Notice('开始分P不能大于结束分P');
              return;
            }
            if (endP - startP + 1 > 100) {
              new Notice('一次最多处理100个分P');
              return;
            }
        ...
        if (localFile && localFile.includes(',')) {
          ...
          if (isSingleLink && singleUrl) {
            ...
            const snippet = providedTranscript
              .trim()
              .split('\n')[0]
              .substring(0, 12)
              .replace(/[<>:"/\\|?*\s]/g, '_');
            ...
            const fileName =
              localFile.split('/').pop() || localFile.split('\\').pop() || 'local_file';
  ...
  private async renderCombinedFileListAndHistory(container: HTMLElement) {
    ...
    const sortFiles = () => {
      ...
      const filtered = videoFiles.filter(({ file, status }) => {
        ...
        const statusMatch =
          statusSelected === 'all' ||
          normalized === statusSelected ||
          (statusSelected === 'success' && normalized === 'success');
  ...
  private renderCombinedSortedFileList(
    container: HTMLElement,
    sortedFiles: Array<{ file: TFile; status: string; content: string }>,
  ) {
    // 移除现有的文件列表
  ...
  private renderLinkInput(container: HTMLElement) {
    ...
    generateBtn.onclick = async () => {
      ...
      const isMultiP = multiPCheckbox.checked;
      const singleUrl = isMultiP
        ? this.cleanVideoUrl(rawSingleUrl)
        : this.cleanVideoUrlKeepP(rawSingleUrl);
      // 获取输出文件夹，如果未设置则使用当前活跃文件所在文件夹
      let outputFolder = this.plugin.settings.outputFolder || '';
      if (!outputFolder) {
        const activeFile = this.plugin.app.workspace.getActiveFile();
  ...
  // 新增方法: 批量生成分P到队列
  private async generatePartsFromRangeToQueue(
    url: string,
    startP: number,
    endP: number,
    providedTranscript: string,
  ...
  private async generatePartsFromRange(
    url: string,
    startP: number,
    endP: number,
    providedTranscript: string,
  ...
  private renderPartsList() {
    ...
    const partsSection =
      (this as any).partsSectionEl ||
      (this.containerEl.querySelector('.parts-section') as HTMLElement);
  ...
  private async batchProcessSelectedParts() {
    const selectedParts = this.videoParts.filter(
      (part) => (part as any).checkbox && (part as any).checkbox.checked,
    );
...
`;
  ...
  private async batchProcessFiles(files: TFile[]) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        let link = '';
        if (
          (metadata as any).link &&
          Array.isArray((metadata as any).link) &&
          (metadata as any).link.length > 0
        ) {
        ...
        const localFile =
          (metadata as any).local_file ||
          (metadata as any).localFileName ||
          (metadata as any).localFile;
        if (localFile) {
  ...
  private renderFileGroup(
    container: HTMLElement,
    files: Array<{ file: TFile; status: string; content: string }>,
    groupType: string,
  ): HTMLElement {
  ...
  private renderHistoryList(container: HTMLElement, history: any[]) {
    ...
    history.forEach((record) => {
      ...
      const modeText =
        record.mode === 'summary' ? '总结' : record.mode === 'transcript-only' ? '文稿' : '信息';
      const langText =
        record.language === 'zh'
          ? '中文'
          : record.language === 'en'
  ...
  private async handleGetLastResult() {
    try {
      ...
      history.forEach((entry, index) => {
        ...
        item.createEl('div', {
          text: `${index + 1}. ${new Date(entry.timestamp).toLocaleString('zh-CN')} · ${
            entry.mode === 'summary' ? '总结' : entry.mode === 'transcript-only' ? '文稿' : '信息'
          } (${entry.language})`,
          cls: 'history-meta',
        });
        if (entry.input.url) {
  ...
  private async createNoteFromWebhookResult(entry: {
    ...
    try {
      const folder =
        ...
        this.plugin.app.workspace.getActiveFile()?.parent?.path ||
        '';
      const baseTitle =
        entry.result.video_title ||
        (entry.input.url
          ? this.generateVideoTitle(
              entry.input.url,
