...
export class PlaylistProcessor {
  ...
  private static parseUrl(url: string): {
    type: 'favlist' | 'playlist';
    id: string;
    idParam: 'fid' | 'sid' | 'media_id';
  } {
    // 收藏夹链接 - 支持多种格式
   ...
   * 提取收藏夹内容
   */
  private static async extractFavlist(
    id: string,
    idParam: 'fid' | 'media_id' = 'media_id',
   ...
   * 提取播放列表内容
   */
  private static async extractPlaylist(
    id: string,
    idParam: 'sid' | 'media_id' = 'sid',
  ): Promise<PlaylistInfo> {
    try {
      // 首先获取播放列表基本信息
